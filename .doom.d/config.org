...
** Window pop up rules
I like certain things to pop up on the right hand side 50% of the screen.

#+begin_src emacs-lisp
(set-popup-rule! "^\\*eww*" :side 'right :size 0.50 :select t :ttl nil)
(set-popup-rule! "^\\*notmuch*" :side 'bottom :size 0.50 :select t :ttl nil)
...
** Org/Agenda Settings
Other agenda settings. Can't get time grid to work, would really like that... please send help..
...
(define-key global-map (kbd "C-c ;") #'now)
#+end_src
